...
const ForkTsCheckerWebpackPlugin =
  process.env.TSC_COMPILE_ON_ERROR === "true"
    ...
    : require("./ForkTsCheckerWebpackPlugin");
...
const themesDir = fs
  .readdirSync(paths.keycloackThemesPath, {
    withFileTypes: true,
  })
...
for (const themeDir of themesDir) {
  ...
  for (const pageDir of pagesDir) {
    const templatesDir = fs
      .readdirSync(path.join(paths.keycloackThemesPath, themeDir, pageDir))
      .filter((dir) => dir.endsWith("-template"));
    for (const templateDir of templatesDir) {
      const ftlTemplates = fs
        .readdirSync(
          path.join(paths.keycloackThemesPath, themeDir, pageDir, templateDir)
        )
        .filter((dir) => dir.endsWith(".ejs"));

      const componentsPages = fs
        .readdirSync(
          path.join(paths.keycloackThemesPath, themeDir, pageDir, templateDir)
        )
        .filter((dir) => dir.endsWith("page.tsx"));

      if (ftlTemplates.length && componentsPages.length) {
        const ftlTemplate = ftlTemplates[0];
        const componentPage = componentsPages[0];

        keycloackTemplates.push({
          templateSrc: path.join(
            paths.keycloackThemesPath,
            themeDir,
            pageDir,
            templateDir,
            ftlTemplate
          ...
          templateOut: path.join(
            themeDir,
            pageDir.replace("-pages", ""),
            ftlTemplate
          ),
          entry: {
            chunk: ftlTemplate.replace(".ejs", ""),
            path: path.join(
              paths.keycloackThemesPath,
              themeDir,
              pageDir,
  ...
  if (keycloackTemplates.length) {
    themes.push({
      themeDir: themeDir,
      templates: keycloackTemplates,
    });
  }
}
console.dir(themes);
...
module.exports = function (webpackEnv) {
  ...
  return themes.map((theme) => ({
    ...
    entry: theme.templates.reduce((previous, current) => {
      // previous[current.entry.chunk] = [current.entry.path];
    ...
    output: {
      ...
      assetModuleFilename: theme.themeDir + "/js/[name].[hash][ext]",
      ...
      rules: [
        ...
        },
        {
          ...
          oneOf: [
            ...
            //   { test: /\.ftl$/,   exclude: /\.ftl$/,  type: 'asset/source' },
    ...
    plugins: [
      ...
      isEnvProduction &&
        new MiniCssExtractPlugin({
          ...
          chunkFilename:
            theme.themeDir + "css/[name].[contenthash:8].chunk.css",
        }),
